...
import { Backdrop, Collapse, Divider, Grid, Grow, Input, Paper, Stack } from "@mui/material";
import { Button, CircularProgress, CssVarsProvider, IconButton, Input as JoyInput } from "@mui/joy";
...
const UploadDownload: React.FC<UploadDownloadProps> = ({
                                                         downloadLink,
                                                         setDownloadEnabled,
                                                         downloadEnabled,
                                                         outputExists,
  ...
  return (
    <Grid container sx={containerStyle}>
            ...
            <Grid container display={"flex"} flexDirection={"row"} justifyContent={"space-between"}
              ...
              <Grid item flex={2} alignSelf={"flex-end"}>
                <Button disabled={!outputExists}
                        style={{width: "100%", backgroundColor: `${outputExists ? "#23C0AD" : "grey"}`, color: "white"}}
                        type={"submit"} onClick={handleSubmit}>Convert to Json</Button>
              </Grid>
            </Grid>

            <Grid item sx={uploadWrapperStyle} width={"100%"}>
              <FileUploadRoundedIcon sx={{position: "absolute", fontSize: "2rem"}} fontSize={"large"}/>
              <Input required inputProps={{accept: ".xlsx"}} ref={uploadRef} sx={hideUpload} itemID={"upload"}
                     type="file" id={"file"} onInputCapture={handleChange}/>
            </Grid>
            ...
            <Stack sx={{width: "100%", height: "100%"}}>
              <Collapse in={options}>
                ...
                <Grid item display={"flex"} flexDirection={"column"} width={"100%"} flex={1}>
                  ...
                  <JoyInput variant={"outlined"} size={"sm"} id={"set-header"} type={"text"} value={header}
      ...
      <Grid container display={"flex"} flexDirection={"row"} margin={"auto"} gap={1} width={"30%"}
                ...
                {downloadEnabled ?
                  ...
                  <Grid container sx={{display: "flex", flexFlow: "column", gap: 1}}>
                    ...
                    <Button type={"reset"} onClick={handleReset}
                            startDecorator={<RefreshIcon sx={{color: "white", fontSize: "2rem"}}/>}>
                      New File
                    </Button>
